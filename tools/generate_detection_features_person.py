...
def demo(sess, net, frame_name,v_fram_dir,saveDirectory,det_file):
    """Detect object classes in an image using pre-computed object proposals."""
    # Load the demo image
    frame_file = os.path.join(v_fram_dir,frame_name)
    ...
    for cls_ind, cls in enumerate(CLASSES[2]):
        ...
        with open(saveDirectory + '/' + det_file,'a') as f:
            for i in inds:
                ...
                f.write('{:d},'.format(-1))
                for index,v_bbox in enumerate(bbox):
                     if index in [0,1]:
                        f.write("{:d},".format(int(v_bbox)))
                     elif index == 2:
                        f.write("{:d},".format(int(int(bbox[2])-int(bbox[0]))))
                     else:
                        f.write("{:d},".format(int(int(bbox[3])-int(bbox[1]))))
...
if __name__ == '__main__':
    ...
    for frame in frames:
        ...
        try:
           demo(sess, net, frame,video_frames_directory,saveDirectory,det_file)
